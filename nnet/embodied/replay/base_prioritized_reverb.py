        ...
        self.server = reverb.Server(
            tables=[
                reverb.Table(
                    ...
                    signature={key: torch.tensor_spec(shape, dtype) for key, (shape, dtype) in self.signature.items()},
                )
            ],
            port=None,
...
def _split_key(self, key):
    ...
    keyA = (key >> 32) & 0xFFFFFFFF
    keyB = key & 0xFFFFFFFF
    return torch.tensor(keyA, dtype=torch.int32), torch.tensor(keyB, dtype=torch.int32)

def _combine_key(self, keyA, keyB) -> torch.int64:
    """Combine the two 32bit ints into a single 64bit int"""
    return (keyA.to(torch.int64) << 32) + keyB.to(torch.int64)

    return keyA_t * torch.tensor(2**32, dtype=torch.uint64) + keyB_t
