...
def train_eval(agent, train_env, eval_env, train_replay, eval_replay, logger, args):
    ...
    driver_train.on_episode(lambda ep, ep_info, worker: per_episode(ep, mode="train"))
    driver_train.on_step(lambda tran, info, _: step.increment())
    driver_train.on_step(train_replay.add)
    driver_eval = embodied.Driver(eval_env)
    driver_eval.on_step(eval_replay.add)
    driver_eval.on_episode(lambda ep, ep_info, worker: per_episode(ep, mode="eval"))
    ...
    policy_train = lambda *args: agent.policy(*args, mode="explore" if should_expl(step) else "train")
